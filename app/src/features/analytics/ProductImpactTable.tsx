...
const DynamicImpactBadge = (params: any) => {
  ...
  return (
    <div className="flex items-center gap-2 h-full font-mono">
      <div className={`px-2 py-0.5 border text-[10px] font-bold tracking-tighter ${isMvp ? "bg-black text-white border-black" : "bg-white text-black border-black"}`}>
    ...
    <Card id='product_impact_table' className="h-[90vh] w-full flex flex-col border-none shadow-none rounded-none bg-white">
      <CardHeader className="px-6 py-4 border-b-2 border-black rounded-none">
        <div className="flex justify-between items-baseline text-black">
          <div>
            <CardTitle className="text-3xl font-black uppercase tracking-tighter italic">Performance Audit</CardTitle>
            <CardDescription className="text-black font-bold text-[10px] uppercase tracking-[0.2em] opacity-50">Automated Supplier Benchmarking // Data-Driven</CardDescription>
          </div>
