  ...
  return (
    <AppShell>
      <div className="relative h-full w-full flex flex-col overflow-hidden bg-white">
        ...
        <div className={`
          z-20 transition-all duration-700 ease-[cubic-bezier(0.23,1,0.32,1)]
          ${stage === "results" 
            ? "pt-10 pb-4" 
            : stage === "searching"
              ? "absolute inset-x-0 top-[10vh] flex flex-col items-center pointer-events-none" // Move up when searching to keep visible
        ...
        <div className="flex-1 w-full min-h-0 relative">
          {stage === "results" && (
            <div className="absolute inset-0 w-full mx-auto h-full animate-in fade-in duration-700">
              <SearchResultsTable />
            </div>
          )}
        </div>
        ...
        <div
          className={`
            z-50 transition-all duration-700 ease-[cubic-bezier(0.23,1,0.32,1)]
            ${stage === "results"
                ? "py-6 bg-white" 
                : stage === "searching"
                  ? "absolute inset-x-0 top-[25vh] flex flex-col items-center pointer-events-none" // Align with the shifted title
        ...
        >
          <div
            className={`
              bg-white w-1/3  mx-auto transition-all duration-500 overflow-hidden pointer-events-auto
              ${stage === "results"
